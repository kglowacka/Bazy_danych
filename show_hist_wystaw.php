...
//    }
    ...
    $wynik = pg_exec($link, "SELECT id_eksponatu, tytul
				FROM Eksponaty
				WHERE (obecna_lokalizacja!='S') AND ((LOWER(tytul) LIKE LOWER('%$tytul%')) AND (id_eksponatu=$id OR $id=0)");
    ...
    if($ile==0){
        ...
        echo "<h2 align=center>Oto historia wystaw dla $nazwa</h2>";
        $dane = pg_exec($link, "SELECT Historia_ekspozycji.id_wydarzenia, Historia_ekspozycji.poczatek, Historia_ekspozycji.koniec, Sale.nr_galerii, Sale.nr_sali
				FROM Eksponaty
				...
				WHERE Eksponaty.id_eksponatu=$numer AND historia_ekspozycji.id_wystawy IS NOT NULL
				ORDER BY poczatek");
        ...
        <table border="1" align=center>
            ...
            for($i = 0; $i < $il_wierszy; $i++) {
                ...
                echo "<td>" . $wiersz["id_wydarzenia"] . "</td>";
