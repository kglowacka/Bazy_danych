...
<div class="col-sm-11 text-left">
    ...
    $wynik = pg_exec($link, "SELECT Eksponaty.id_eksponatu, Eksponaty.tytul, Eksponaty.typ, historia_ekspozycji.poczatek, Magazyn.planowany_koniec
			      FROM Eksponaty LEFT JOIN historia_ekspozycji
			      ON Eksponaty.id_eksponatu=historia_ekspozycji.id_eksponatu
			      LEFT JOIN Magazyn
			      ON historia_ekspozycji.id_magazynowania = Magazyn.id_magazynowania
			      WHERE historia_ekspozycji.koniec IS NULL AND Eksponaty.obecna_lokalizacja='M'");
    $ile=pg_numrows($wynik);
    if ($ile==0) {
        echo "<center><strong>Brak eksponatów w magazynie!</strong></center>";
    }
    else{
        ?>
        <h2 align=center>Eksponaty w magazynie</h2>
        <table border="1" align=center>
        <tr>
            <th>ID</th>
            <th>Tytuł</th>
            <th>Typ</th>
            <th>Początek magazynowania</th>
