...
//    }
    ...
    $koniec = $_POST["koniec"];
    $format = explode("-", $poczatek);
    $poczatek = $format[1] . "/" . $format[2] . "/" . $format[0];
    $format = explode("-", $koniec);
    $koniec = $format[1] . "/" . $format[2] . "/" . $format[0];
    $link = pg_connect("host=$dbhost dbname=$dbname user=$dbuser password=$dbpass");
    $wynik = pg_exec($link, "SELECT Eksponaty.id_eksponatu, Eksponaty.tytul, Eksponaty.typ, Historia_ekspozycji.id_wystawy, Historia_ekspozycji.nr_sali, Historia_ekspozycji.id_wystawy, Historia.poczatek, Historia_ekspozycji.koniec
				FROM Eksponaty LEFT JOIN historia_ekspozycji
				ON Eksponaty.id_eksponatu=historia_ekspozycji.id_eksponatu
				WHERE LOWER(Eksponaty.tytul) LIKE LOWER('%$tytul%') AND (Eksponaty.id_eksponatu=$id OR $id=0) AND (historia_ekspozycji.poczatek <= '$koniec' AND historia_ekspozycji.koniec >= '$poczatek')eksponaty.obecna_lokalizacja!='S'");
    $il_wierszy=pg_numrows($wynik);
    if($il_wierszy==0){
        echo "<center><strong>Błąd!</strong></center>";
        echo $poczatek;
        echo " ";
        echo $koniec;
    }
        ...
        for($i = 0; $i < $il_wierszy; $i++) {
	      ...
	      <td>" . $wiersz["tytul"] . "</td>
	      <td>" . $wiersz["typ"] . "</td>";
            if (!is_null($wiersz["nr_sali"])) echo " <td> Galeria </td>";
            else if (!is_null($wiersz["id_magazynowania"])) echo " <td> Magazyn </td>";
            else if (!is_null($wiersz["id_wystawy"])) echo " <td> Wystawa objazdowa </td>";
