...
//    }
    ...
    if($ile==0){
        ...
        echo "<h2 align=center>Oto historia magazynowania dla $nazwa</h2>";
        $dane = pg_exec($link, "SELECT Historia_ekspozycji.id_magazynowania, Historia_ekspozycji.poczatek, Historia_ekspozycji.koniec
				FROM Eksponaty
				LEFT JOIN historia_ekspozycji
				ON Eksponaty.id_eksponatu=historia_ekspozycji.id_eksponatu
				WHERE Eksponaty.id_eksponatu=$numer AND historia_ekspozycji.id_magazynowania IS NOT NULL
				ORDER BY poczatek");
        ...
        for($i = 0; $i < $il_wierszy; $i++) {
            ...
            echo "<td>" . $wiersz["id_magazynowania"] . "</td>";
